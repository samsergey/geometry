...
(defun ghcid-stack-cmd (target)
  (format "stack repl %s --ghci-options=-fno-code" target))
...
;; TODO Pass in compilation command like compilation-mode
(defun test-command (h)
    (format "stack test --file-watch --coverage" (ghcid-stack-cmd ghcid-target) h))
...
you ran this command from."
  (interactive)
  (ghcid-start-test default-directory))
