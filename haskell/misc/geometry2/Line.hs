...
instance Curve Line where
  ...
  param l t = let (p1, p2) = refPoints l
              ...
              in case bounding l of
                   ...
                   Semibound -> if 0 <= t
                                then Right p
                                else Left $ "param: Parameter lies out of parameter interval for " <> show l
                   Bound -> if 0 <= t && t <= 1 
                            then Right p
                            else Left $ "param: Parameter lies out of parameter interval for " <> show l

  project l p
    | isTrivial l = Left $ "project: Line " <> show l <> " is trivial."
    | otherwise =
        ...
        in case bounding l of
             ...
             Semibound -> if 0 ~<= res
                          then Right res
                          else Left $ "The projected point doesn't belong to " <> show l
             Bound -> if 0 ~<= res && res ~<= 1
                      then Right res
                      else Left ("The projected point doesn't belong to " <> show l)
  ...
  tangent l _ = Right (angle l)

  distanceTo p l = case p ->@ l of
    Right x -> p `distance` (l @->! x)
    Left _ -> (p `distance` (l @->! 0)) `min`
               (p `distance` (l @->! 1))
