...
function pointOrXY (pos) {
    console.assert(pos instanceof Point || isCoordinates(pos),
...
class Figure {
    ...
    at (point_or_xy) {
	return this.superpose(this.refPoint, pointOrXY(point_or_xy).point)
    }
...
class Point extends BoxedFigure {
    ...
    at (point_or_xy) {
	const res = this.copy()	
	res.xy = point_or_xy instanceof Point ? point_or_xy.xy : point_or_xy
	return res
    }
...
class Angle extends BoxedFigure {
	...
	//tested
	const res = this.copy()
	res.vertex = pointOrXY(point_or_xy).point
	return res
    }
...
class Polygon extends Curve {
    ...
    addVertex (point_or_xy, n = this.number) {
	var res = this.copy()
	res.vertices.insert(n, pointOrXY(point_or_xy).point)
	res.resetEquations()
	return res
    }
    ...
    at (point_or_xy, vertex_n = 1) {
	return this.superpose(this.vertex(vertex_n), pointOrXY(point_or_xy).point)
    }
...
class Circle extends Curve {
    ...
    at (point_or_xy) {
	const res = this.copy()
	res.center = pointOrXY(point_or_xy).point
	res.resetEquations()
	return res
    }
...
class Text extends BoxedFigure {
    ...
    at (point_or_xy) {
	const res = this.copy()
	res.pivot = pointOrXY(point_or_xy).point
	return res
    }
