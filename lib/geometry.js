...
function lineEquation([x1, y1], [x2, y2], scale = 1) {
    // tested
    var v = [x2 - x1, y2 - y1];
    return t => [x1, y1].vadd(v.scale(t*scale));
}
...
class Point extends Figure {
	...
	this.xy = s1.intersection(s2)[n || 0].xy;
	return this;
    }
	...
	if (Number.isFinite(this.x) && Number.isFinite(this.y)) {
	    p.listPlot([this.xy], this.options);
	
	    if (this.isLabeled) {
		var l = this.option('label');
		var lp = this.xy.vadd([-0.3*l.length, -0.5]);
		label(p, l, lp.vadd(this.option('labelOffset')));
	    }
	}
    }
}
...
class Line extends Figure {
	...
	p.line([this.eqn(t1), this.eqn(t2)], this.options);
...
class Ray extends Line {
	...
	p.line([this.eqn(0), this.eqn(t2)], this.options);
...
class Segment extends Line {
    ...
    show(p)
    {
	p.line([this.eqn(0), this.eqn(1)], this.options);
