...
function appendBox (box1, box2) {
    return [[min(box1[0][0], box2[0][0]), min(box1[0][1],box2[0][1])]
	    ,[max(box1[0][0], box2[0][0]), max(box1[0][1],box2[0][1])]]
}

var Box = {mappend: appendBox, mempty:[[Infinity,Infinity],[-Infinity,-Infinity]]}
...
class Group extends Figure {
    ...
    get box () {
	return this.contents.foldMap(Box)
    }
...
class Line extends Figure {
	...
	return [this.eqn(ts[0]).xy, this.eqn(ts[1]).xy]
    }
...
class Ray extends Line {
	...
	return [this.eqn(0).xy, this.eqn(t).xy]
    }
...
class Segment extends Line {
    ...
    flip () {
	return this.reflectAt(this.middle)
    }

    get box () {
	const ts = plane.intersections(this).map(p => this.location(p)).sorted();
	if (ts.length < 2) return;
	return [this.eqn(max(0,ts[0])).xy, this.eqn(min(1,ts[1])).xy]
    }
...
class Text extends Figure {
    ...
    angle (a) {
	return this.option('angle',a || 1e-12);
    }
	...
	if (this.isLabeled) {
	    ...
	    mkLabel(paper, l, lp, Object.assign(this.options,{'style':style.text}));
	}
    }
