...
class any {
    ...
    /** Arbitrary number */
    static number (min = -paperSize/2, max = paperSize/2) {
	return {
	    'generator' : () => lRandom(min,max),
	    'shrink' : numShrink(0.1)
	}
    }

    static get num () {	return this.number() }

    /** Arbitrary positive number */
    static positiveNumber (min = -paperSize/2, max = paperSize/2) {
	return {
	    'generator' : () => lRandom(0, max),
	    'shrink' : numShrink(1)
	}
    }

    /** Arbitrary integer number */
    static natural (min = 1, max = paperSize/2) {
	return {
	    'generator' : () => lRandom(min, max, round),
	    'shrink' : numShrink(1)
	}
    }
    ...
    /** Arbitrary point */
    static get point () {
	return this.iso([this.xy],
			point, p => [p.xy])
    }

    /** Arbitrary line */
    static get line () {
	return this.iso([any.point, any.point],
			line, l => [l.point(0), l.point(1)])
    }

    /** Arbitrary ray */
    static get ray () {
	return this.iso([any.point, any.point],
			ray, l => [l.point(0), l.point(1)])
    }

    /** Arbitrary ray */
    static get segment () {
	return this.iso([any.point, any.point],
			segment, l => [l.point(0), l.point(1)])
    }

    /** Arbitrary circle */
    static get circle () {
	return this.iso([any.point, any.positiveNumber()],
			circle, c => [c.center, c.R])
    }

    //------------------------------------------------------------
    // combinators

    static iso (gen, constr, destr) {
	return {
	    'generator' : () => lMap(constr, this.tuple(gen).generator()),
	    'shrink' : xs => lMap(constr,
				  lProduct(gen.zipWith(destr(xs), (g, n) => () => g.shrink(n)))),
	}
    }
    
    static tuple (xs) {
	return {
	    'generator' : () => lZip(xs.map(x => x.generator())),
...
function numShrink(tol) {
	...
	let s = Math.sign(b)
	if (s == -1) yield (-b)
	let x = 0, max = tol*round (abs(b)/tol)
	while( abs(x - max) > tol ) {
	    x = tol*round ((x + max)/2/tol)
	    yield s*x
	}
    }
}
