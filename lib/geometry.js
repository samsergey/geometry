...
class Curve extends BoxedFigure {
    ...
    norm (s) { return rot90(this.tangent(s).normalize()) }
...
class RegularPolygon extends Polygon {
	...
	const c = new Circle(r),
	      pts = new Polygon().through(range(n).map(i => c.point(i*2*pi/n)))
	      .rotate(90)
...
class Triangle extends Polygon {
	...
	if (!point1) {
	    ...
	    s = new RegularPolygon(3).on(segment(point1,point2))
	} else {
...
class Circle extends Curve {
    ...
    get radius () {
	return segment(this.center, this.point(0))
    }
