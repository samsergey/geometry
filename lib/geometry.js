...
class Point extends BoxedFigure {
	...
	console.assert(pos instanceof Point || isXY(pos),
		       "Point.iso: position shoud be either Point or coordinates")
...
////////////////////////////////////////////////////////////
// Curve
/** Representing a parameterized curve
 * @extends BoxedFigure
 */
class Curve extends BoxedFigure {
    ...
    //------------------------------------------------------
    /** Resets methods [<b>point</b>]{@link Curve#point}, [<b>equation</b>]{@link Curve#equation}
    ...
    /** Returns a unit vector, tangent to a curve at a given location.
     * @param {Number} s
     * @return {XY}
     */
    tangentV (s) {
    ...
    /** Returns a line, tangent to a curve at a given location. 
     * The refference point of a line coinsides with the location.
     * @param {Number} s
     * @return {Line}
     */
    tangent (s) { return new Line().at(this.point(s), this.tangentV(s)) }
    
    /** Returns a unit vector, normal to a curve at a given location. 
     * @param {Number} s
     * @return {XY}
     */
    normalV (s) { return rot90(this.tangentV(s).normalize()) }

    /** Returns a line, normal to a curve at a given location. 
     * The refference point of a line coinsides with the location.
     * @param {Number} s
     * @return {Line}
     */
    ...
    /** Returns an angle of the tangent line at a given location. 
     * @param {Number} s
     * @return {angle}
     */
...
class Line extends Curve {
	 ...
	 */
	this.pivot = [0, 0]
	/** The refference vector of a line
	 * @type {XY}
	 */
	this.vector = [1, 0]
	/** @default 'infinite' */
    ...
    location (pos) {
	var v = Point.iso(pos).xy.vsub(this.pivot)
	return v.dot(this.vector)/this.vector.norm()**2
    }

    isContaining (pos) {
	return equal(cross(this.vector, Point.iso(pos).xy.vsub(this.pivot)), 0)
    }
	...
	const res = this.copy(),
	      v = vector(pos1, pos2)
	res.pivot = Point.iso(pos1).xy
	res.vector = v
	res.resetEquations()
	return res
    }

    /** Returns a line, passing through a point with a given refference vector.
     * @return {Line}
     * @param {Point | XY} pos
     * @param {XY} v
     */
    at (pos, v = this.vector) {
	const res = this.copy()
    ...
    /** Returns a line tangent to a given circle.
     * @return {Line}
     * @param {Circle} circle
     */
	...
	return line instanceof Line
	    && equalMod(180, line.angle(0),  this.angle(0))
    }
	...
	return line instanceof Line
	    && equalMod(180, line.angle(0) - this.angle(0), 90)
    }
...
class Segment extends Line {
	...
	if (this.isParallelTo(line))
	    return new Ray().at(this.start).parallelTo(this)
	else
	...
	if (obj instanceof Curve) {
	    let r = new Ray().at(this.start).parallelTo(this),
...
class Angle extends BoxedFigure {
    ...
    /** Isomorphism between number and unit vector
     * @static
     * @param {angle | XY} obj
     ...
     */
    static iso (x) {
	console.assert(Number.isFinite(x) || isXY(x),
		       "Angle.iso: angle shoud be either number or a vector")

	if (isXY(x))
	    return {vector : x, angle : angleV(x)}
	let phi = deg2rad(x)
...
class Polygon extends Curve {
    ...
    location (pos) {
	let i = this.sides.map(s => s.location(pos)).findIndex(l => 0 <= l && l <= 1)
	console.assert(i != -1,
		       `Point [${pos.xy}] can't be located on a curve ${this.label()}.`)
	let s = this.sides[i],
	    l0 = this.verticesLocations[i],
	    l1 = s.location(pos)*s.length/this.length
	return l0 + l1
    }
...
class Circle extends Curve {
    ...
    location (pos) {
	return deg2rad(line(this.center, pos).angle(0))/(2*pi)
    }
    ...
    intersectionL (line) {
	const h = new Segment().at(this.center).heightTo(line)
	if (equal(h.length, this.R))
	    return [h.end]
	if (h.length > this.R)
	    return []
	const x = sqrt(this.R**2 - h.length**2),
	      s = new Segment().at(h.end).parallelTo(line).extend(-x,x)
...
////////////////////////////////////////////////////////////
// smart constructors
/** Representing an arbitrary exemplar of an object */
class any {
    /** Arbitrary number */
    static number (max = 50) {
	return {
	    'special' : [0,1,-1,max,-max],
	    'sample' : () => max*(2*Math.random() - 1)
	}
    }
	...
	return {
	    'special' : [''],
	    'sample' : () => range(3).map(() => ['a','b','c','1','2','3'].sample()).sum()
	}
    }
    ...
    static get angle () {
	return {
	    'special' : [0,45,90,180,360,-90,-180],
	    'sample' : () => 360*(2*Math.random() - 1)
	}
    }

    /** Arbitrary coordinates */
    static get xy () {
	return this.pair(this.number(25), this.number(25))
    }

    /** Arbitrary point */
    static get point () {
	return this.map(point, this.xy)
    }

    /** Arbitrary line */
    static get line () {
	return this.map(line, any.xy, any.either(any.angle, any.xy))
    }
    ...
    static either (...xs) {
	return {
	    'special' : [...xs].mapappend(x => x.special),
	    'sample' : () => [...xs].sample().sample()
	}
    }
    
    /** pair of arbitrary objects */
    static pair (x, y) {
	return {
	    'special' : x.special.outer(y.special),
    ...
    static tuple (xs) {
	return {
	    'special' : cartesianProduct(xs.map(x => x.special)),
	    'sample' : () => xs.map(x => x.sample())
	}
    }
    ...
    static map (f, ...xs) {
	return {
	    'special' : cartesianProduct([...xs].map(x => x.special), f),
	    'sample' : () => f.apply(null, [...xs].map(x => x.sample()))
	}
    }

    /** arbitrary samples */
    static samples(x, n = 10) {
	return x.special.concat(range(n).map(x.sample))
    }   
}
...
function line (pos1, pos2 = 0) {
	...
	res.pivot = Point.iso(pos1).xy
	if (isPosition(pos2))
	    res.vector = vector(pos1, pos2)
	else
	    res.vector = polarVector(pos2)
    } else
	res.vector = polarVector(pos1)
    res.resetEquations()
    return res
}
