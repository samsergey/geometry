...
function naturalParametrization(pts, type = 'infinite') {
    ...
    let l = ls.last()
    let X = linearInterpolation([ls.scale(1/l),xs].transpose())
    let Y = linearInterpolation([ls.scale(1/l),ys].transpose())
...
class Figure {
    ...
    reflectT (a) { return [[cos(2*a), sin(2*a), 0],
			   [sin(2*a), -cos(2*a), 0],
			   [0, 0, 1]] }
    translateT ([dx, dy]) { return [[1, 0, dx], [0, 1, dy], [0, 0, 1]] }
    ...
    translate(vector) {
	// tested Point
	return this.transform(this.translateT(vector))
    }
...
function row(figures, alignment = 0, space = 0) {
    return figures.foldr((el,res) => el.beside(res, alignment, space))
}

function column(figures, alignment = 0, space = 0) {
...
class Point extends BoxedFigure {
    ...
    get box () { return [[this.x, this.y], [this.x, this.y]] }
...
class Curve extends BoxedFigure {
    ...
    locate (point) {
	console.error('locate is not implemented')
    }
    ...
    isInside (point) { return undefined  }

    tangent (s) {
	var df = differential(t => this.point(t).xy)
	return df(s).normalize()
    }
    norm (s) { return rot90(this.tangent(s)) }
    angle (s) { return angleV(this.tangent(s)) }

    isIntersecting (obj) {
	return obj instanceof Curve
	    && this.intersections(obj)
	    .filter(p => this.isContaining(p) && obj.isContaining(p))
    }

    setUnit (scale = 1) {
	console.assert(!equal(scale, 0),
		       "Internal scale of the curve can't be equal zero." )
	let res = this.copy()
	res.unit = scale
	res.resetEquations()
	return res
...
class Line extends Curve {
	...
	[x0,y0] = this.pivot;
	[vx,vy] = this.vector;
	this.point = s => point([x0 + this.unit * s * vx,
				 y0 + this.unit * s * vy])
    ...
    tangent (t) { return this.vector }

    location (point) {
	return vector(this.point(0), point).dot(this.tangent(0))/this.unit
    }
	...
	console.assert(obj instanceof Curve,
		       "Intersections could be found only for curves.")
	if (obj instanceof Line)
	    return [point(intersectionV(this.pivot, this.vector, obj.pivot, obj.vector))]
	if (obj instanceof Circle)
	...
	const d = seg(this.point(0), circle.center)
	...
	console.assert(line instanceof Line,
		       "isCCW coul be computed only for lines")
	...
	if (this.isLabeled)
	{
	    const lp = this.point(ts[1]).xy.vsub(this.tangent(0).scale(2))
	    const lpos = lp.vadd([-0.5, -0.5])
	    const loff = this.norm(0).scale(this.option('labelOffset'))
...
class Ray extends Line {
	...
	return (obj instanceof Ray)
	    ...
	    && equal(this.tangent(0),obj.tangent(0))
    }
	...
	if (this.isLabeled)
	{
	    const lp = this.point(t).xy.vsub(this.tangent(0).scale(2))
	    const lpos = lp.vadd([-0.5, -0.5])
	    const loff = this.norm(0).scale(this.option('labelOffset'))
	    mkLabel(paper, this.option('label'), lpos.vadd(loff))
	}
...
class Segment extends Line {
	...
	// tested
	const p = this.pivot.vadd(this.tangent(0).scale(number))
	...
	const ts = plane.intersections(this).map(p => this.location(p)).sorted()
	if (ts.length < 2) return
	paper.line([this.point(max(0,ts[0])).xy, this.point(min(1,ts[1])).xy], this.options)
	...
	if (this.option('mark') > 0)
	{
		...
		const m = this.point(1/2).xy.vadd(this.tangent(0).scale(0.2*i))
		paper.line([m.vadd(this.norm(0).scale(0.3)), 
			    m.vadd(this.norm(0).scale(-0.3))], 
			   {'class':'thin'})
	    }
	}
    }
...
class Angle extends BoxedFigure {
	...
	if (this.isRight)
	{
	    ...
	    const p1 = this.startRay.tangent(0)
	    const p2 = this.endRay.tangent(0)
	...
	if (this.isLabeled)
	{
	    ...
	    const lpos = new Ray().bisectrisse(this).tangent(0).scale(r)
...
class Polygon extends Curve {
	...
	return this.type == 'closed'
	    && (this.isContaining(point)
		|| this.sides.same(s => s.isCCW(line(s.end, point))))
    }

    rescale (s) {
	console.assert(!equal(s, 0), "Curve scale can't be equal zero." )
	let res = this.copy()
	let pts = res.vertices.map(p => p.xy)
	const par = naturalParametrization(pts, res.type)
	res.point = par.eqn
	res.length = par.length
	return res
    }
...
class RegularPolygon extends Polygon {
	...
	const c = new Circle(r),
	      pts = new Polygon().through(range(n).map(i => c.point(i*2*pi/n - pi/n/2)))
...
class Sector extends Polygon {
	...
	this.vertices = range(startAngle, endAngle + 2, 2)
	    .map(a => c.point(deg2rad(a))).concat([center])
	this.resetEquations()
    }
...
class Circle extends Curve {
    ...
    get radius () {
	return seg(this.center, this.point(0))
    }
	...
	res.orientation = Math.sign(cross(r.vector, this.trans(T,t)))
	res.resetEquations()
	return res
    }
	...
	const c = this.center,
	      ...
	      ph = this.phase,
	      w = this.orientation
	this.point = t => point([c.x+this.R*cos(w*t + ph),
				 c.y+this.R*sin(w*t + ph)])
	...
	const ph = this.phase,
	      w = this.orientation
	return [-w*sin(w*t + ph), w*cos(w*t + ph)]
    }

    location (point) {
	return deg2rad(line(this.center, point).angle(0))
    }
	...
	const a = (r0**2 - r1**2)/(2*R),
	      ...
	      m = r.middle.xy.vadd(r.tangent(0).scale(a))
	if (b == 0)
	    return [point(m.vadd(r.norm(0).scale(b)))]
	else 
	    return [point(m.vadd(r.norm(0).scale(b))), point(m.vadd(r.norm(0).scale(-b)))]
    }
...
class Text extends BoxedFigure {
	...
	return res.atAngle(line.angle(0)).option('labelOffset',line.norm(0).scale(2/3))
    }
...
class Scale extends BoxedFigure {
    constructor(steps = 2, scale = x => x, format = x => `${x}`) {
	super()
	const defaults = {
	    'tickLength' : 0.5,
	    'scaleF' : scale,
    ...
    scaling (func) { return this.option('scaleF', func) }
    ...
    flip () { return this.reflectAt(this.curve.middle) }
	...
	const tick = x => {
		...
		l = s.end.invisible().label(this.format()(this.scaling()(x)))
		.labelOffset(s.tangent(0).scale(abs(this.tickLength())))
...
function circularScale(n) {
    return new Scale(n-1, t => (n-1)*t/(2*pi-2*pi/n), fmt.int)
	.range([0, 2*pi-2*pi/n])
}
