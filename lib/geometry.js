...
function naturalParametrization(pts, type = 'infinite') {
    ...
    let l = ls.last()
    let X = linearInterpolation([ls,xs].transpose())
    let Y = linearInterpolation([ls,ys].transpose())

    if (type == 'closed') 
	res = s => [X(mod(s, l)), Y(mod(s, l))]
    else
	res = s => [X(s), Y(s)]
...
class Line extends Curve {
	...
	// tested
	const res = this.copy(),
	      x = this.trans(T, this.pivot.vadd(this.vector))
    ...
    location (point) {
	// tested
	return point.xy.vsub(this.pivot).dot(this.tangent(0))/this.vector.norm()
    }
	...
	return obj instanceof Curve
	    && this.intersections(obj).filter(p => this.isContaining(p)
					      && obj.isContaining(p))
    }
...
class Segment extends Line {
	...
	this.equation = ([x,y]) => equal((x - x0)/vx, (y - y0)/vy)
	return this
    }
