...
class Figure {
	...
	// tested Point
	const pt = point.xy;
	return this.shift(pt.flip()).stretch(line, scale).shift(pt);
    }
    ...
    align(line1, line2) {
	// tested Point
	return this.rotateAt(line1.eqn(0), line2.angle(0) - line1.angle(0));
    }
    ...
    isEqual (obj) { return undefined; }

    isSimilar (obj) { return undefined; }
...
class Curve extends BoxedFigure {
    constructor (label) {
	super(label);
	this.eqn = t => point([0,0]);
    }

    tangent (t) {
	console.error('tangent is not implemented');
    }

    norm (t) {
	console.error('norm is not implemented');
    }
    
    isContaining (point) {
	console.error('isContaining is not implemented');
    }
...
class Line extends Curve {
    ...
    tangent (t) { return this.vector.normalize(); }
    norm (t) { return [-this.tangent(t)[1], this.tangent(t)[0]]; }
    angle (t) { return angleV(this.tangent(0)); }
    ...
    location (pnt) {
	// tested
	return pnt.xy.vsub(this.pivot).dot(this.tangent(0))/this.vector.norm();
    }

    isContaining (point) {
	// tested
	return abs(cross(point.xy.vsub(this.pivot), this.tangent(0))) < 1e-13;
    }
	...
	// tested
	if (obj instanceof Line)
	    return [point(intersectionV(this.pivot, this.tangent(0), obj.pivot, obj.tangent(0)))];
	if (obj instanceof Circle)
	...
	return Box.mappend(this.eqn(ts[0]).box, this.eqn(ts[1]).box);
    }
	...
	if (this.isLabeled)
	{
	    const lp = this.eqn(ts[1]).xy.vsub(this.tangent(0).scale(2));
...
class Ray extends Line {
	...
	return Box.mappend(this.eqn(0).box, this.eqn(t).box);
    }
	...
	if (this.isLabeled)
	{
	    const lp = this.eqn(t).xy.vsub(this.tangent(0).scale(2));
	    const lpos = lp.vadd([-0.5, -0.5]);
	    const loff = this.norm(0).scale(this.option('labelOffset'));
	    mkLabel(paper, this.option('label'), lpos.vadd(loff));
	}
...
class Segment extends Line {
	...
	const ts = plane.intersections(this).map(p => this.location(p)).sorted();
	if (ts.length < 2) return Box.mempty;
	return Box.mappend(this.eqn(max(0,ts[0])).box, this.eqn(min(1,ts[1])).box);
    }
	...
	if (this.option('mark') > 0)
	{
		...
		const m = this.eqn(1/2).xy.vadd(this.tangent(0).scale(0.2*i));
...
class Circle extends Curve {
	...
	const res = new Circle(r.length).at(r.start);
	...
	const c = this.center,
	      ph = this.phase;
	this.eqn = t => point([c.x+this.R*cos(t + ph),
			       c.y+this.R*sin(t + ph)]);
    }

    location (p) {
	return deg2rad(line(this.center, p).angle(0));
    }

    tangent (t) { return [-sin(t + this.phase), cos(t + this.phase)]; }
    norm (t) { return [cos(t + this.phase), sin(t + this.phase)]; }
    angle (t) { return angleV([-sin(t + this.phase), cos(t + this.phase)]); }
    ...
    get box () {
	return range(0,2*pi,pi/2).foldMap(t => this.eqn(t).box, Box);
    }
...
function circularScale(n) {
    return new Scale(n-1, t => (n-1)*t/(2*pi-2*pi/n), fmt.int).range([0, 2*pi-2*pi/n]);
}
