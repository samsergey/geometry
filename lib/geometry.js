...
class Point extends Figure {
	...
	return this.at(curve1.intersection(curve2)[n].xy);
    }
...
class Line extends Figure {
    ...
    isParallelTo (line) { return abs (line.angle - this.angle) % 180 == 0; }
...
class Segment extends Line {
    ...
    extendToLine(line)
    {
	return this.joining(this.start, line.intersection(this)[0]);
    }
    ...
    tangentTo(circle, direction = 1) {
	var t = super.tangentTo(circle, direction);
	if (circle.isContaining(point(this.pivot))) {
	    return this.parallelTo(t);
	} else {
	    var d = point(this.pivot).distance(circle.center);
	    var p = this.extendToLength(sqrt(d**2-circle.R**2)).end;
	    var a = c.location(p);
	    return this.joining(point(this.pivot), circle.eqn(a));
	}
    }
    
    intersection (curve) {
	return super.intersection(curve).filter(p => this.isContaining(p));
    }
...
class Angle extends Figure {
    ...
    on (line) {
	var v = this.value;
	this.vertex = point(line.pivot);
	this.start = line.angle;
