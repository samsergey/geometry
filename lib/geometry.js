...
class Polygon extends Figure {
    ...
    contains (p) {
	return this.segments.some(s => s.contains(p))
    }
    ...
    intersection (that) {
	return this.segments.mapappend(s => s.intersection(that))
    }
...
class Circle extends Figure {
    ...
    contains (p) {
	return abs(p.pt.vsub(this.center).length - this.radius) < 1e-14
    }
