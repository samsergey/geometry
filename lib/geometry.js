...
class Circle extends Curve {
	...
	const c = this.center,
	      ph = this.phase,
	      w = this.orientation;
	this.eqn = t => point([c.x+this.R*cos(w*t + ph),
			       c.y+this.R*sin(w*t + ph)]);
    }

    location (p) { return deg2rad(line(this.center, p).angle(0)); }
    tangent (t) {
	const ph = this.phase,
	      w = this.orientation;
	return [-w*sin(w*t + ph), w*cos(w*t + ph)];
    }
    norm (t) { return rot90(this.tangent(t)); }
    angle (t) { return angleV(this.tangent(t)); }
