...
class any {
	...
	return {
	    'generator' : () => gRandom(min, max, round),
	    'shrink' : numShrink(1)
	}
    }
	...
	return {
	    'generator' : () => gMap(constr, this.tuple(gen).generator()),
	    'shrink' : xs => gMap(constr,
				  gProduct(gen.zipWith(destr(xs), (g, n) => () => g.shrink(n)))),
	}
    }
    
    static tuple (xs) {
	return {
	    'generator' : () => gZip(xs.map(x => x.generator())),
	    'shrink' : ns => gProduct(xs.zipWith(ns, (x, n) => () => x.shrink(n))),
	}
    }
}
