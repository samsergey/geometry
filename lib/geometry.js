...
function Point(label,opts)
{
	...
	this.x = x
	this.y = y
	label_pt = this.pt.vadd([-0.3*this.label.toString().length,-0.5])
	return this
    }
    
    this.onLine = function (line,t)
    {
	this.at(line.eqn(t))
	return this
    }

    this.between = function(p1,p2,t)
    {
...
class Segment {
	...
	this.angle = angleV(this.unit)
	this.label_pt = this.middle.vadd([-0.5,-0.5])
    ...
    {
	if (this.length == 0)
	    console.error('segment has zero length')
	var p1 = this.eqn(0)
	var p2 = this.eqn(1)
    ...
    labelPosition(t)
    {
	this.label_pt = this.eqn(t).vadd([-0.5,-0.5])
	return this
    }
    
    labelOffset(o)
    {
	this.labelOffset = o.normalize().scale(1.25)
	return this
    }
...
class Line extends Segment {
    ...
    constructor(label,opts) {
	super(label,opts)
	super.joining(point([-100,0]), point([100,0]))
	this.angle = 0
	...
	var s = new Segment().joining(point(eqn(-100)), point(eqn(100)))
	this.pivot = p1
	this.start = s.start
	this.end = s.end
	this.eqn = eqn
...
/*
function Ray(label,opts)
{
    this.joining = function (p1, p2)
    {
	var eqn = lineEquation(p1.pt, p2.pt)
	var res = new Segment(label, opts).joining(p1, point(eqn(100)))
	res.eqn = eqn
	res.middle = undefined
	res.vector = undefined
	res.length = undefined
	return res
    }
    
    this.through = function (p, a)
    {
	var r = deg2rad(a)
	var p2 = point(p.pt.vadd([cos(r),sin(r)]))
	return this.joining(p,p2)
