...
class Curve extends BoxedFigure {
    ...
    isEnclosing (point) { return false  }
...
class Segment extends Line {
    ...
    clipBy (curve) {
	if (!(curve instanceof Curve) || curve.type != 'closed')
	    return this
	let p1 = this.start, p2 = this.end, l = [p1, p2]
	if (curve.isEnclosing(p1) && curve.isEnclosing(p2))
	    l = [p1, p2]
	else if (curve.isEnclosing(p1)) 
	    l = [p1, curve.intersections(this)[0] || p2]
	else if (curve.isEnclosing(p2)) 
	    l = [curve.intersections(this)[0] || p1, p2]
	else
	    l = curve.intersections(this)
	if (l.length == 2)
	    return this.joining(l[0], l[1])
	else
    ...
    show (paper)
    {
	let s = this.clipBy(plane)
	if (s instanceof EmptyFigure) return
...
class RegularPolygon extends Polygon {
	...
	const c = new Circle(r),
	      pts = new Polygon().through(range(n).map(i => c.point(i/n)))
	      .rotate(90)
