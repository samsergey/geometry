...
class Segment extends Line {
    ...
    show (paper)
    {
	let p1 = this.start, p2 = this.end, l
	if (plane.isEnclosing(p1) && plane.isEnclosing(p2))
	    l = [p1.xy, p2.xy]
	else if (plane.isEnclosing(p1)) 
	    l = [p1, plane.intersections(this)[0]]
	else if (plane.isEnclosing(p2)) 
	    l = [plane.intersections(this)[0], p2]
	else
	    l = plane.intersections(this).map(p => p.xy)
	
	paper.line(l, this.options)
