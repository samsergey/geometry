...
class Text extends BoxedFigure {
	...
	return res.atAngle(line.angle).option('labelOffset',line.norm.scale(2/3));
    }
...
class Scale extends BoxedFigure {
    ...
    transform (T) {
	const res = this.copy();
	res.axis = this.axis.transform(T);
	return res;	
    }
    ...
    get group () {
	const tick = x => {
	    let p = this.axis.eqn(x);
	    let s = new Segment().through(p)
		.perpendicularTo(this.axis)
		.extendToLength(0.5*this.orientation);
	    let l = s.end.label(this.format(this.scale(x)))
		.labelOffset(s.unit.scale(0.5*this.orientation));
	    return new Group([s,l]);
	};

	return new Group(range(0,1+1/this.steps,1/this.steps).map(tick));
    }
