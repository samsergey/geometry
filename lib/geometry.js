...
class Figure {
    ...
    stretch(line, scale) {
	// tested Point
	return this.rotate(-line.angle).scale(1,scale).rotate(line.angle);
    }
...
class BoxedFigure extends Figure {
    ...
    get boxWidth () {
	return 	this.boxCorner.lowerLeft.distance(this.boxCorner.lowerRight);
    }
    
    get boxHeight () {
	return 	this.boxCorner.lowerLeft.distance(this.boxCorner.upperLeft);
    }
    
    get boxCorner () {
	let xmin,ymin,xmax,ymax;
	[[xmin,ymin],[xmax,ymax]] = this.box;
	return { lowerLeft : point([xmin, ymin]),
		 lowerRight : point([xmax, ymin]),
		 upperLeft : point([xmin, ymax]),
		 upperRight : point([xmax, ymax]),
		 upperMiddle : point([(xmin+xmax)/2, ymax]),
		 lowerMiddle : point([(xmin+xmax)/2, ymin]),
		 leftMiddle : point([xmin, (ymin+ymax)/2]),
		 rightMiddle : point([xmax, (ymin+ymax)/2]),
	       };
    }

    get lowerLeft () {return this.boxCorner.lowerLeft ;}
    get lowerRight () {return this.boxCorner.lowerRight ;}
    get upperLeft () {return this.boxCorner.upperLeft ;}
    get upperRight () {return this.boxCorner.upperRight ;}
    get lowerMiddle () {return this.boxCorner.lowerMiddle ;}
    get upperMiddle () {return this.boxCorner.upperMiddle ;}
    get rightMiddle () {return this.boxCorner.rightMiddle ;}
    get leftMiddle () {return this.boxCorner.leftMiddle ;}
    
    
    beside (obj, alignment = 'center', space = 0) {
	let res = this.copy();
	switch (alignment) {
	case 'lower' :
	    res = this.superpose(this.lowerRight, obj.lowerLeft);
	    break;
	case 'center':
	    res = this.superpose(this.rightMiddle, obj.leftMiddle);
	    break;
	case 'upper' :
	    res = this.superpose(this.upperRight, obj.upperLeft);
	    break;
	};
	return res.shift([-space, 0]);
    }

    above (obj, alignment = 'center', space = 0) {
	let res = this.copy();
	switch (alignment) {
	case 'right' :
	    res = this.superpose(this.lowerRight, obj.upperRight);
	    break;
	case 'center':
	    res = this.superpose(this.lowerMiddle, obj.upperMiddle);
	    break;
	case 'left'  :
	    res = this.superpose(this.lowerLeft, obj.upperLeft);
	    break;
	};
	return res.shift([0, space]);	
    }
}
...
class Line extends BoxedFigure {
    constructor(label) {
	super(label);
	const defaults = {
	    'points': false,
...
class Polygon extends BoxedFigure {
    constructor(label) {
	super(label);
	const defaults = {
...
class Circle extends BoxedFigure {
    constructor(r = 1) {
	super();
	const defaults = {
	    'style': style.circle
...
class Text extends BoxedFigure {
	...
	return res.angle(line.angle).option('labelOffset',line.norm.scale(2/3));
    }
    ...
    get box () {
	return new Rectangle(this.length, 1)
	    .at(this.pivot.xy)
	    .atAngle(this.angle)
	    .box;
    }
	...
	if (this.isLabeled) {
	    ...
	    mkLabel(paper, l, lp, Object.assign(this.options,
						{'style' : style.text
