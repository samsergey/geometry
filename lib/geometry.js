...
class Segment extends Line {
	...
	if (obj instanceof Curve) {
		...
		return this.through(int[0])
	}
	return this
    }
    
    /** Returns a segment perpendicular to a given line with current starting point.
     * If there is no intersection the ray is formed.
     * @return {Segment | Ray}
     * @param {Line|Ray|Segment} line
     */
    heightTo (line) {
	// tested
	return this.perpendicularTo(line).extend(line)
    }
...
class Angle extends BoxedFigure {
    ...
    isContaning (pos) {
	return (isPosition(pos)
		&& (this.startRay.isContaning(pos)
		    || this.endRay.isContaning(pos)))
    }
...
class any {
    /** Arbitrary number */
    static number (max = paperSize/2) {
	return {
	    'special' : () => lList([0,1,-1,max,-max]),
	    'sample' : () => lRandom(r => max*(2*r - 1)),
	    'shrink' : numShrink(0.1)
	}
    }
    
    /** Arbitrary positive number */
    static positiveNumber (max = paperSize/2) {
	return {
	    'special' : () => lList([0,1,max]),
	    'sample' : () => lRandom(r => max*r),
	    'shrink' : numShrink(0.1)
	}
    }

    /** Arbitrary integer number */
    static integer (max = paperSize/2) {
	return {
	    'special' : () => lList([0,1,-1,max,-max]),
	    'sample' : () => lRandom(r => round(max*(2*r - 1))),
	    'shrink' : numShrink(1)
	}
    }
    ...
    static get angle () {
	return {
	    'special' : () => lList([0,90,-90,180,360]),
	    'sample' : () => lRandom(r => 360*(2*r - 1)),
	    'shrink' : empty
	}
    }
    ...
    static get xy () {
	return this.tuple([this.number(), this.number()])
    }
    ...
    /** Arbitrary line */
    static get line () {
	return this.map(line, any.xy, any.xy)
    }

    /** Arbitrary ray */
    static get ray () {
	return this.map(ray, any.xy, any.xy)
    }

    /** Arbitrary ray */
    static get segment () {
	return this.map(segment, any.xy, any.xy)
    }

    /** Arbitrary circle */
    static get circle () {
	return this.map(circle, any.xy, any.positiveNumber())
    }
    ...
    static tuple (xs) {
	return {
	    'special' : () => lProduct(xs.map(x => x.special)),
	    'sample' : () => lInvoke(xs.map(x => x.sample())),
	    'shrink' : ns => lProduct(xs.zipWith(ns, (x, n) => () => x.shrink(n))),
	}
    }
    ...
    static map (f, ...xs) {
	return {
	    'special' : () => lMap(f, lZip([...xs].map(x => x.special()))),
	    'sample' : () => lMap(f, lInvoke([...xs].map(x => x.sample()))),
	    'shrink' : n => empty
	}
    }
}
...
function* lRandom(f = id) {
    for(;;) yield f(Math.random())
}
...
* @example
* lProduct([() => llist([10,20]),
*          () => llist([1,2,3]), 
*          () => llist(['a','b'])])
*/
function* lProduct(gs) {
    if (gs.length == 0)	yield []
    else if (gs.length == 1) yield* gs[0]()
    else 
	for(let x of gs[0]()) 
