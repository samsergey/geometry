...
('Transformations',
 [ new test("shift 3", new Line().shift([1,2]).pivot, [1,2])
   ...
   , new test("shift 8", seg1.copy().shift([5,6]).end.xy, [9,12])
...
('Group',
 [ new test("elements 1", grp1.element(1).option('label'), 'A')
   , new test("elements 2", grp1.element(6) instanceof Circle, true)
   , new test("elements 3", grp1.element(8).xy, [1,2])
   , new test("copy", range(1,8).every(i => grp2.element(i).isEqual(grp1.element(i))),true)
 ])
...
('Point',
 [ new test("coordinates 1", new Point().xy, [0,0])
   ...
   , new test("copy 2", pnt1.isEqual(origin.copy().shift(pnt1.xy)),true)
   ...
   , new test("inside 1", pnt1.isInside, true)
   , new test("inside 2", pnt1.shift([100,100]).isInside, false)
 ])
