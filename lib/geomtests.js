...
('Points',
 [new Properties([
	 ...
	 name : "isomorphism 1",
	 for : [any.point],
	 require : p => equal(Point.iso(Point.iso(p).xy).point, p)},
     {
	 name : "isomorphism 2",
	 for : [any.xy],
	 require : xy => equal(Point.iso(Point.iso(xy).point).xy, xy)},
     {
	 name :"copy 1",
	 for : [any.point],
	 require : p => p.copy().isEqual(p)},
     {
	 name :"copy 2",
	 for : [any.point, any.string],
	 require : (p, s) => p.label(s).copy().label() == s},
     {
	 name :"coordinates 1",
	 for : [any.either(any.point, any.xy)],
	 require : p => origin.at(p).isEqual(point(p))},
     {
	 name :"equality 1",
	 for : [any.point],
	 require : p => p.isEqual(p)},
     {
	 name :"equality 2",
	 for : [any.point],
	 require : p => point(p.xy).isEqual(p)},
     {
	 name :"equality 3",
	 for : [any.point],
	 require : p => !p.copy().translate([1,1]).isEqual(p)}
 ])

 ]);

////////////////////////////////////////////////////////////
const testAngles = new testSuite
('Angle computations',
 [new Properties([
     {
	 name : "modulus",
	 for : [any.angle],
	 require : a => equal(mod(-a, 360), mod(360-a, 360))},
     {
	 name : "isomorphism 1",
	 for : [any.xy],
	 require : v => equal(Angle.iso(Angle.iso(v).angle).vector, v.normalize())},
     {
	 name : "isomorphism 2",
	 for : [any.angle],
	 require : a => equalMod(360,Angle.iso(Angle.iso(a).vector).angle, a)}
 ])]);

////////////////////////////////////////////////////////////
const testLines = new testSuite
('Lines',
 [new Properties([
     {
	 name     : "isomorphism 1",
	 ...
	 require  : (l, s) => equal(l.locus(l.point(s)), s)},
     {
	 name     : "isomorphism 2",
	 for      : [any.line, any.number()],
	 assuming : [l => l.isTrivial],
	 require  : (l, s) => equal(l.locus(l.point(s)), 0)},
     {
	 name     : "isomorphism 3",
	 for      : [any.line, any.number()],
	 assuming : [l => !l.isTrivial],
	 require  : (l, s) => { let p = l.point(s)
	 ...
	 assuming : [l => l.isTrivial],
	 require  : (l, s) => { let p = l.point(s)
				return equal(l.point(l.locus(p)).xy, p.xy) }},
     {
	 name     : "equation 1",
	 for      : [any.line, any.number()],
	 require  : (l, s) => l.equation(l.point(s)) },
     {
	 name     : "equation 2",
	 for      : [any.line, any.number()],
	 assuming : [l => !l.isTrivial],
	 require  : (l, s) => !l.equation(l.point(s).translate(l.normalV(0))) },
     {
	 skip     : true,
	 name     : "line intersections 1",
	 for      : [any.line, any.line],
	 assuming : [(a,b) => !(a.isTrivial || b.isTrivial)],
	 require  : (a,b) => {let ps = a.intersections(b)
			      return a.isParallelTo(b) ? ps.length == 0 : ps.length == 1 }},
     {
	 name     : "plane intersections 1",
	 for      : [any.line],	     
	 require  : l => {let ps = plane.intersections(l)
			  return ps.every(p => plane.isContaining(p)) }},
     {
	 name     : "perpendicularity 1",
	 for      : [any.line, any.xy],
	 assuming : [l => !l.isTrivial],
	 require  : (l, xy) => new Line().at(xy).perpendicularTo(l).isPerpendicularTo(l) },
     {
	 name     : "perpendicularity 2",
	 for      : [any.line],
	 require  : l => !l.isPerpendicularTo(line(origin, origin)) },
     {
	 name     : "parallelity 1",
	 for      : [any.line, any.xy],
	 assuming : [l => !l.isTrivial],
	 require  : (l, xy) => new Line().at(xy).parallelTo(l).isParallelTo(l) },
     {
	 name     : "parallelity 2",
	 for      : [any.line],
	 require  : l => !l.isParallelTo(line(origin, origin)) },
     {
	 name     : "tangentTo 1",
	 for      : [any.point, any.circle],
	 require  : (p, c) => {let t = new Line().at(p).tangentTo(c)
			       let C = t.intersections(c)[0]
			       new Line().at(C).perpendicularTo(t).isContaining(c.center)} },

 ])])

////////////////////////////////////////////////////////////
const testRays = new testSuite
('Rays',
 [new Properties([
     {
	 name     : "isContaining 1",
	 ...
	 for      : [any.ray, any.number()],
	 assuming : [r => !r.isTrivial],
	 require  : (r, s) => !r.isContaining(r.point(s).translate(r.normalV(0))) }
     
 ])])

const testSegments = new testSuite
('Segments',
 [new Properties([
     {
	 name     : "isContaining 1",
	 ...
	 name     : "extension 1",
	 for      : [any.segment, any.number()],
	 require  : (s, n) => equal(s.extend(n).length, abs(n)*s.length) },
     {
	 name     : "extension 2",
	 for      : [any.segment, any.number()],
	 require  : (s, n) => equal(s.extend(n).extend(n).length, n*n*s.length) },
     {
	 name     : "extension 3",
	 for      : [any.segment, any.number()],
	 assuming : [(_,n) => n >= 0],
	 require  : (s, n) => equal(s.extendToLength(n).length, s.isTrivial ? 0 : n) },
     {
	 name     : "extension 4",
	 for      : [any.segment],
	 require  : (s, n) => s.extend(0).isTrivial && s.extendToLength(0).isTrivial}
...
const testPoints = new testSuite
('Point',
 [ new Property("copy 1", [any.point], p => p.copy().isEqual(p))
   , new Property("copy 2", [any.point, any.string], (p, s) => p.label(s).copy().label() == s)
   , new Property("coordinates 1", [any.either(any.point, any.xy)],
		  p => origin.at(p).isEqual(point(p)))
   , new Property("equality 1", [any.point], p => p.isEqual(p))
   , new Property("equality 2", [any.point], p => point(p.xy).isEqual(p))
   , new Property("equality 3", [any.point], p => !p.copy().translate([1,1]).isEqual(p))
