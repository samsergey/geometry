...
('Angle',
 [ new testSuite
   ...
   , new test("shift 1", ang1.shift([10,20]).vertex.xy, [11,22])
   , new test("shift 2", ang1.shift([3,4]).value, ang1.value)
   , new test("shift 3", ang1.shift([10,20]).start, lin1.angle)
   , new test("scale 1", ang1.scale(10,10).vertex.xy, [10,20])
   , new test("scale 2", ang1.scale(10,10).value, ang1.value)
   , new test("scale 3", ang1.scale(10,10).start, lin1.angle)
   , new test("rotate 1", ang1.rotateAt(point([1,2]),10).vertex.xy, ang1.vertex.xy)
   , new test("rotate 2", ang1.rotate(10).value, ang1.value)
   , new test("rotate 3", ang1.rotate(10).start, lin1.angle + 10)
   ...
   , new test("reflect 7", ang1.reflectAt(lin1).start, lin1.angle)
   , new test("reflect 8", ang1.reflectAt(lin1).end, lin1.angle-ang1.value)
   ...
   , new test("within 1", new Angle().within(cir1.eqn(pi/6), cir1.center, cir1.eqn(pi/3)).value, 30)
   , new test("within 2", new Angle().within(cir1.eqn(pi/3), cir1.center, cir1.eqn(pi/6)).value, 330)
   , new test("within 3", new Angle().within(cir1.eqn(pi/2), cir1.eqn(2), cir1.eqn(3*pi/2)).isRight)
   , new test("within 4", new Angle().within(cir1.eqn(pi/6), cir1.center, cir1.eqn(pi/6)).value, 0)
   ...
   , new test("between 1", new Angle().between(Ox,lin1).value, lin1.angle)
...
//   , new test("isEqual 1", pol1.copy().isEqual(new Polygon().through([pnt1, pnt2, origin])))
   ...
   , new test("area 1", tri1.area.isContaining(tri1.median(3).eqn(0)))
   , new test("area 2", tri1.area.isContaining(tri1.median(3).middle))
   , new test("area 3", tri1.area.isContaining(tri1.median(3).eqn(1)))
   , new test("area 4", !tri1.area.isContaining(tri1.median(3).eqn(2)))
   , new test("area 5", !tri1.area.isContaining(tri1.median(3).eqn(-1)))
   , new test("area 6", new Square(2).area.isContaining(origin))
   , new test("area 7", new Square(2).area.isContaining(point([1,0])))
   , new test("area 8", !(new Square(2).area.isContaining(point([2,0]))))
   , new test("curve 1", tri1.curve.isContaining(tri1.median(3).eqn(1)))
   , new test("curve 2", tri1.curve.isContaining(tri1.median(3).eqn(0)))
   , new test("curve 3", !tri1.curve.isContaining(tri1.bisectrisse(1).eqn(1/2)))
   , new test("shift")
   , new test("scale")
   , new test("superpose")
   , new test("rotate")
...
sqr1 = new Square(2)
var testSquare = new testSuite
('Square',
 [ new test("identification 1", new Square(2).isCurve)
   ...
   , new test("constructor 1", new Square(2).segments.same(s => s.length))
   , new test("constructor 1", new Square(2).side(1).length, 2)
   , new test("constructor 1", new Square().through(pnt1, pnt2).side(1).middle, [2,3])
 ])
...
('Circle',
 [ new test("identification", cir1.isCurve),
   new test("copy 1", seg1.copy().start.xy, seg1.start.xy)
   , new test("copy 2", seg1.copy().end.xy, seg1.end.xy)
   , new test("area 1", cir1.area.isContaining(cir1.center))
   , new test("area 2", cir1.area.isContaining(cir1.center.shift([cir1.R/2,0])))
   , new test("area 3", cir1.area.isContaining(cir1.center.shift([cir1.R-1e-12,0])))
   , new test("area 4", cir1.area.isContaining(cir1.center.shift([cir1.R,0])))
   , new test("area 5", !cir1.area.isContaining(cir1.center.shift([cir1.R+1e-12,0])))
   , new test("curve 1", cir1.curve.isContaining(cir1.eqn(1)))
   , new test("curve 2", cir1.curve.isContaining(cir1.center.shift([cir1.R,0])))
   , new test("curve 3", !cir1.curve.isContaining(cir1.center.shift([cir1.R-1e-12,0])))
   , new test("curve 4", !cir1.curve.isContaining(cir1.center.shift([cir1.R+1e-12,0])))
   , new test("shift")
   , new test("scale")
   , new test("superpose")
   , new test("rotate")
