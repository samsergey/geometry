...
const testSegments = {
	...
	},
	{
	    ...
	    with : (s, r) => [ s, r, s.heightTo(r) ],
	    hold : (s, r, h) =>  (h instanceof Segment
	...
	},
	{
	    ...
	    with : x => { let C = new Circle(10),
			      ...
			      t = new Segment('tg').at(A).tangentTo(C).flip(0).extendTo(Ox),
			      ct = new Segment('ctg').at(A).tangentTo(C).extendTo(Oy),
	    ...
	    assuming : [ x => !equal(mod(x , 1/4), 0) ]
	},
	{
	    name : "tangentTo 1",
	    for : [anyPoint(), anyCircle()],
	    with : (p, c) => [p, c, new Segment().at(p).tangentTo(c)],
	    hold : (p, c, t) => t .isParallelTo (c.tangent(c.locus(t.end))),
	    assuming : [(p, c) => c.isNontrivial && !c.isEnclosing(p)]
	},
	{
	    name     : "tangentTo 2",
	    for      : [anyPoint(), anyCircle()],
	    with     : (p, c) => {let t = new Segment().at(p).tangentTo(c)
				  return [p, c, t]},
	    hold     : (p, c, t, i) => (c .isContaining (t.end) &&
					new Line().at(t.end).perpendicularTo(t).isContaining(c.center)),
	    assuming : [(p, c) => c.isNontrivial && !c.isEnclosing(p)]
	},
	{
	    name     : "tangentTo 3",
	    for      : [anyCircle(), anyParam()],
	    with     : (c, x) => [c, x, new Segment().at(c.point(x)).tangentTo(c), c.radius(x)],
	    hold     : (c, x, t) => t.isPerpendicularTo(c.radius(x)),
	    assuming : [ c => c.isNontrivial ]
	},
	{
	    name     : "tangentTo 4",
	    for      : [anyPoint(), anyCircle()],
	    with     : (p, c) => {let t1 = new Segment().at(p).tangentTo(c, 1),
				      t2 = new Segment().at(p).tangentTo(c, -1)
				  return [p, c, t1, t2]},
	    hold     : (p, c, t1, t2) => (t1.vector.dot(c.tangentV(c.locus(t1.end))) > 0 &&
					  t2.vector.dot(c.tangentV(c.locus(t2.end))) < 0),
	    assuming : [(p, c) => c.isNontrivial && !c.isEnclosing(p)]
	},
...
const testCircle = {
	...
	{name : 'tangent',
	 suite :[
	     {name: '1', run : () => new Circle(3).tangentV(0), result : [0,1] },
	     {name: '2', run : () => new Circle(3).tangentV(1/2), result : [0,-1] },
	     {name: '3', run : () => new Circle(3).tangentV(1/4), result : [-1,0] },
	     {name: '4', run : () => new Circle(3).tangentV(1/8), result : [-1,1].normalize() },
	     {name: '5', run : () => new Circle(3).tangentV(1), result : [0,1] }]
	}	
    ]
}
