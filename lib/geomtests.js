...
('Transformations',
 [ new test("shift 3", new Line().shift([1,2]).pivot, [1,2])
   ...
   , new test("shift 7", seg1.copy().shift([5,6]).start.xy, [6,8])
   , new test("shift 8", seg1.copy().shift([5,6]).end.xy, [11,12])
   , new test("shift 9", seg1.copy().shift([5,6]).isParallelTo(seg1))
   , new test("shift 10", cir1.copy().shift([5,6]).center.xy, [6,8])
...
('Point',
 [ new test("identification", pnt1.isCurve, false)
   ...
   , new test("copy 1", pnt1.isEqual(pnt1.copy()))
   , new test("copy 2", pnt1.isEqual(origin.copy().shift(pnt1.xy)))
   , new test("shift 1", new Point().shift([1,2]).xy, [1,2])
   , new test("shift 2", new Point().shift([1,2]).shift([-1,-2]), new Point())
   , new test("scale 1", pnt1.copy().scale(2,3).xy, [2,6])
   , new test("scale 2", pnt1.copy().scaleAt(pnt1,2,3).xy, pnt1.xy)
   , new test("superpose", pnt1.copy().superpose(pnt1, pnt2).xy, pnt2.xy)
   , new test("rotate 1", pnt1.copy().rotate(90).xy, [-2,1])
   , new test("rotate 2", pnt1.copy().rotateAt(pnt1,90).xy, pnt1.xy)
   , new test("rotate 3", pnt1.copy().rotateAt(pnt2,180).xy, [5, 6])
   , new test("reflect 1", pnt1.copy().reflect(0).xy, [1, -2])
   , new test("reflect 2", pnt1.copy().reflect(90).xy, [-1, 2])
   , new test("reflect 3", pnt1.copy().reflect(45).xy, [2, 1])
   , new test("reflect 4", pnt1.copy().reflectAt(point([2,3]), 135).xy, pnt2.xy)
   , new test("reflect 5", pnt1.copy().reflectAt(pnt1, 45).xy, pnt1.xy)
   , new test("reflect 6", pnt1.copy().reflectAt(point([2,3]), 45).xy, pnt1.xy)
   , new test("reflect 7", pnt2.copy().reflectAt(lin1).xy, [2.36, 4.48])
   , new test("inside 1", pnt1.isInside)
   , new test("inside 2", !pnt1.copy().shift([100,100]).isInside)
   ...
   , new test("distance 6", cir1.center.distance(cir1), cir1.radius)
...
('Line',
 [ new test("identification", lin1.isCurve),
   ...
   , new test("shift 1", lin1.copy().shift([4,5]).pivot, [5, 7])
   , new test("shift 2", lin1.copy().shift([4,5]).angle, lin1.angle)
   , new test("scale 1", lin1.copy().scale(4).angle, lin1.angle)
   , new test("scale 2", lin1.copy().scale(4).pivot, [4,8])
   , new test("scale 3", lin1.copy().scale(1,-1).angle, 360-lin1.angle)
   , new test("scale 4", lin1.copy().scale(1,2).pivot, [1,4])
   , new test("superpose", lin1.copy().superpose(pnt1,pnt2).pivot, [3,4])
   , new test("rotate 1", lin1.copy().rotate(90).pivot, [-2,1])
   , new test("rotate 2", lin1.copy().rotate(90).vector, [-4,3])
   , new test("rotate 3", lin1.copy().rotateAt(point(lin1.pivot),35).pivot, lin1.pivot)
   , new test("reflect")
 ])
