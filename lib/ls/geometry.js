...
function deg2rad(x) { return x*(pi/180) }
function rad2deg(x) { return 180*x/pi }
...
class Angle extends BoxedFigure {
    /** Creates an angle with given value (against horizon) or a label
     * @param {String | angle} value
     * @return {Angle}
     */
    ...
    get isTrivial () {
	return equal(this.value, 0)
    }

    get refPoint () { return this.vertex }

    /** Locates an angle on a line.
     * @param {Line} line
    ...
    /** Returns an angle with current vertex and start, 
    ...
    /** Returns an angle with given vertex and current start and end
     * @param {Position} pos
     * @return {Angle} a copy of a caller
     */
    at (pos) {
    ...
    get value () { return mod(this.end - this.start, 360) }
    get startRay () {
	...
	const res = this.copy()
	res.start = this.end
	res.end = mod(this.start + 180, 360)
	return res
    }
    ...
    toString () {
	return `<Angle at (${this.vertex.xy}) from ${this.start} to ${this.end})>`
    }
...
class Circle extends Curve {
    ...
    toString () {
	return `<Circle ${this.R} (${this.center.xy})>`
    }
}
...
function ray (pos1, pos2 = 0) {
    ...
    if (isPosition(pos1) && isPosition(pos2))
	return new Ray().joining(pos1, pos2)
    if (isPosition(pos1) && Number.isFinite(pos2))
	return new Ray().at(pos1).atAngle(pos2)
    if (Number.isFinite(pos1))
	return new Ray().atAngle(pos1)
}
...
*/
function segment (pos1, pos2=0) {
    console.assert((isPosition(pos1) || Number.isFinite(pos1))
		   && isPosition(pos2) || Number.isFinite(pos2),
		   "Segment must be specified by a position or an angle.")
    if (isPosition(pos1) && isPosition(pos2))
	return new Segment().joining(pos1, pos2)
    if (isPosition(pos1) && Number.isFinite(pos2))
	return new Segment().at(pos1).atAngle(pos2)
    if (Number.isFinite(pos1))
	return new Segment().atAngle(pos1)
}
...
*/
function circle(r, c) {
    console.assert(isPosition(c) && Number.isFinite(r),
		   `Circle must be specified by a center point or an radius. Given r:${r}, c:${c}.`)
