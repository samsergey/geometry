...
(function(){
  var M, C, S, G, failedParams, failedFigures, failedTests, foldJSON, runTests, normalizeTests, flattenTests, runTest, runProperty, reportSuccess, reportFail, checkProperty, Arbitrary, args, ArbitraryNumber, anyNum, anyPos, exports, slice$ = [].slice, arrayFrom$ = Array.from || function(x){return slice$.call(x);};
  M = Monoid;
  C = console;
  S = Sequence;
  G = Gen;
  ...
  Arbitrary = (function(superclass){
    ...
    Arbitrary.prototype.iso = function(constr, destr){
      ...
      x$.shrink = compose$(destr, this.shrink, function(it){
  ...
  anyPos = function(min, max){
    ...
    x$ = xy = Arbitrary.tuple([x, y]);
    x$.simplicity = norm;
    x$.simplest = [0, 0];
    x$.shrink = function(p){
      return (new Sequence)(sortBy(over(curry$(function(x$, y$){
        return x$ - y$;
      }), norm))(xy.shrink(p).take(500).list));
    };
    x$.ascending();
    return x$;
  };
