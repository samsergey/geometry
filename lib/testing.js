...
function runTests (json) {
	...
	foldJSON('number', Monoid.keep, json.number || 100)].fold(Monoid.composition)
    go(normalization(json))
    function go(ts) {
	if (ts.skip) return
...
function runTest(test, log) {
	...
	console.log(`  expected: %c%s`, 'color:blue', expected.toString(),)
...
class Arbitrary extends Sequence {
    ...
    isom (constr) { return destr => this.iso(constr, destr) }
