...
function checkProperty (options) {
	...
	console.log(seq.isEmpty)
	console.log(seq.head)
	console.log(seq.tail.head)
	...
	if(!ok && res.tail.isEmpty)
	    return {ok : ok, sample : res.head}

	if (depth > options.shrinks)
	    return {ok : false, sample : res.head}

	let sh = samples.shrink(res.head)
...
class Arbitrary extends Sequence {
	...
	let res = new Arbitrary()
	res.elements = xs
	res.generator = () => Gen.zip(xs.map(x => x.generator()))
	res.shrink = ns =>
	    new Sequence(Gen.product(xs.zipWith(ns, (x, n) => () => x.shrink(n).generator())))
	return res
    }
	...
	res.generator = () => Gen.map(constr, this.generator())
	res.shrink = x =>
	    new Sequence(
		Gen.map(constr,
			Gen.product(
...
class ArbitraryNumber extends Arbitrary {
	...
	this.max = max
	this.prec = 0.001
	this.generator = () => Gen.rnd(min, max, roundUp(0.001))
	...
	res.prec = prec
	res.generator = () => Gen.rnd(this.min, this.max, roundUp(prec))
	...
	res.prec = this.prec
	res.generator = () => Gen.rnd(min,max,roundUp(this.prec))
