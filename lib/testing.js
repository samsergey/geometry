...
function runTests(json, log = false, n = 100) {
    ...
    if (json.for) { property(json, log); return }
    if (json.suite)
    {
...
function foldJSON (field, mappend, mempty) {
    const go = value => json => {
	let res = json
	if (json.suite) {
	    return Object.assign(res, {suite : json.suite.map(go(mappend(value, json[field] || mempty)))})
	}
	return Object.assign(res, {[[field]] : mappend(value, json[field] || mempty)})
    }
    return json => go(json[field] || mempty)(json)
}
