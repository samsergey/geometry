...
class Sequence {
    ...
    generator () {
	return Gen.prepend(this.head, this.tail())
    }
    ...
    take (n) {
	return new Sequence(Gen.prepend(this.head, Gen.take(n-1, this.tail())))
    }
	...
	res.tail = () => Gen.zip(seqs.map(s => s.tail()))
	res.elements = seqs
	return res
    }
