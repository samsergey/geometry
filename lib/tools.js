...
const tuple = (...x) => [...x]
Array.prototype.outer = function(lst, f = tuple)
{
    var res = []
...
function Property(name, xs, prop, conditions = () => true, number = 10) {
    this.length = 1
    this.run = function () {
	let samples = any.samples(any.tuple(xs), number)
	let s
	for(let i = 0; i<samples.length; i++) {
	    let s = samples[i],
		res = conditions.apply(null, s) ? prop.apply(null, s) : true
	    if (!res) {
...
function conjuction(ps) {
    return (...xs) => ps.every(p => p(...xs))
}

function Properties(data) {
    this.length = data.length
    this.run = function () { data.forEach(d => property(d).run()) }
}
...
function property(data) {
    ...
    if (skip)
	return new trivialTest()
    let name = options['name'],
	xs = options['for'],
	prop = options['require'],
    	conditions = conjuction(options['assuming']),
	number = conjuction(options['number'])
    return new Property(name, xs, prop, conditions, number)
}
