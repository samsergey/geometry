...
class Sequence {
    ...
    take (n) {
	return new Sequence(Gen.cons(this.head, Gen.take(n-1, this.tailGen())))
    }
	...
	return new Sequence(Gen.map(f, g))
    }
    
    map (f) {
	return Sequence.tuple([this]).apply(x => f([x]))
    }

    dropWhile (p) {
	let g = this.generator()
	return new Sequence(Gen.dropWhile(p, g))
    }

    takeWhile (p) {
	let g = this.generator()
	return new Sequence(Gen.takeWhile(p, g))
    }
    ...
    get tail () {
	let res = new Sequence(this.tailGen())
	...
	return new Sequence(Gen.filter(p, this.generator()))	
    }
